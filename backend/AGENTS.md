...
- For source adjustment SQL, keep table definitions and row-loading logic in separate numbered files. Keep adjustment tables lean: store the keys and values needed by downstream joins, and document rationale/facts in the README instead of adding reason, citation, narrative, or generic `adjustment_id` columns.
- Keep source metadata, absent cross-source facts, mappings, and side overrides out of source-shaped CSV ingestion tables. Use `source_file_versions` for real source versions, version-scoped source adjustment tables for absent source facts, and stable source identifiers when mapping rows downstream.
