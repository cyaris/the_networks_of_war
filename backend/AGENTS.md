...
- Keep related columns grouped consistently in source and transformed select lists. For dyadic A/B fields, put the same field for A and B adjacent before moving to the next field: for example, `c_code_a`, `c_code_b`, then `country_name_a`, `country_name_b`, rather than interleaving all A fields before all B fields. For canonical dyad keys, use `war_id`, `c_code_a`, `c_code_b`, `participant_a`, `participant_b` consistently in select lists, `group by`, `order by`, diagnostic columns, and expected tuples. Keep repeated date components in source order by span and component. When both war-transition columns appear, keep `lagging_war` immediately before `leading_war`. For resolved date-span fields, keep `start_date`, `end_date`, `start_year`, and `end_year` together in that order. Keep battle-death columns at the end with actual deaths before estimated values and estimate flags.
...
- Use as few CTEs as practical. Avoid standalone CTEs for one-use projections, packaging, or simple row-shaping that can be folded into the immediately downstream query without duplicating logic, changing table grain, or making the query harder to read. Prefer a CTE over a derived-table subselect when one of those shapes is needed. Do not use derived-table subselects in `from` or `join` clauses.
...
- Keep the missing COW-code value test metadata-driven: any new pipeline table with `c_code`, `c_code_a`, or `c_code_b`
  columns should be covered by the test automatically, and stage-specific tests should be extended when a new stage
  creates such tables outside the existing fixture coverage.
