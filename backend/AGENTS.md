...
- Avoid `initial` in Step 1 table and file names unless there is a later non-initial version in the same pipeline stage. The Step 1 deliverables should use direct names such as `participants`, `dyads`, `dyad_years`, and `wars`.
...
- Keep source download metadata in `backend/manual/source_metadata.json` as a list of source dictionaries keyed by `key`. Do not add redundant metadata fields such as a `table` value when it can be derived as `source_` plus the source key.
...
- Keep an explicit source/ingest file field in source metadata to identify the CSV the pipeline loads. Do not derive the ingest CSV solely from downloads, because source directories can also contain PDFs, DTA files, codebooks, and other supporting files.
...
- For source downloads, list every file that the pipeline should recreate in source metadata. Avoid separate `primary_pdf` concepts when the source subdirectory should contain all relevant downloaded PDFs.
