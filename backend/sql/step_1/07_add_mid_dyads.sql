...
select
    ...
    cow_date(strtyr, strtmnth, strtday, 1, 1) start_date,
    extract(year from cow_date(strtyr, strtmnth, strtday, 1, 1))::integer start_year,
    cow_end_date(endyear, endmnth, endday) end_date,
    extract(year from cow_end_date(endyear, endmnth, endday))::integer end_year
from source_dyadic_mid
where clean_number(war) = 1),
...
select
    ...
    iff(
        nullif(sum(coalesce(battle_deaths_a, 0)), 0) is null
            and sum(coalesce(battle_deaths_est_a, 0)) > 0,
        1,
        0
    ) battle_deaths_est_a,
    iff(
        nullif(sum(coalesce(battle_deaths_b, 0)), 0) is null
            and sum(coalesce(battle_deaths_est_b, 0)) > 0,
        1,
        0
    ) battle_deaths_est_b,
